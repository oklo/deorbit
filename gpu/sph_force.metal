...
kernel void finish(device packed_float3* vel[[buffer(0)]], device const packed_float3* acc[[buffer(1)]],
                   ...
                   constant float& g[[buffer(12)]], constant float& damp[[buffer(13)]],
                   constant uint& n[[buffer(14)]], uint i[[thread_position_in_grid]]){
    if(i>=n || fixed[i]) return;
    float3 v=float3(vel[i])+float3(acc[i])*(0.5f*dt); v.z -= g*(0.5f*dt);
    if(mat[i]==0) v*=damp;                                  // relaxation damping (slab only; damp=1 in impact runs)
    vel[i]=packed_float3(v);
